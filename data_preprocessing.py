...
def get_clients_openimage(args: object, model: torch.nn.Module, image_size: int = 224) -> list[Client]:
    """
    (deprecated) Read CelebA images and get list of clients.
    ...
    assert(os.path.exists(args.openimage_train_path) and os.path.exists(args.openimage_test_path) and os.path.exists(args.openimage_image_path))
    
    with open(args.openimage_train_path, 'r') as train_f:
        train_file = json.load(train_f)
    with open(args.openimage_test_path , 'r') as test_f :
        test_file  = json.load(test_f )
    
    # transformer
    t = ShuffleNet_V2_X2_0_Weights.IMAGENET1K_V1.transforms()

    clients = []

    print('\nloading clients:')
    for user, train_samples in tqdm.tqdm(train_file.items()):
    # for user, train_samples in train_file.items():
        assert(user in test_file)
        test_samples = test_file[user]
        
        # discard a user if it has too few samples
        if len(train_samples) < args.min_sample:
            continue
        ...
        for x, y in train_samples:
            # x = Image.open(args.openimage_image_path + x)
            # if x.mode != 'RGB':
            #     x = x.convert('RGB')
            # x = t(x)
            xs.append(x)
            ys.append(y)
        ys = torch.as_tensor(ys).long()
        train_dataset = Dataset_openimage(xs, ys, args.openimage_image_path)
        ...
        # save GPU memory
        client = Client(args, train_dataset, test_dataset, copy.deepcopy(model), graph_on_cpu = 'Cycle' in args.project)
        clients.append(client)
